...
private[http] object StreamUtils {
  ...
  def transformMultiple(input: Source[ByteString, Any], transformers: immutable.Seq[Flow[ByteString, ByteString, Any]])(implicit materializer: Materializer): immutable.Seq[Source[ByteString, Any]] =
    transformers match {
      ...
      case multiple ⇒
        val (fanoutSub, fanoutPub) = Source.subscriber[ByteString].toMat(Sink.publisher(transformers.size, 16))(Keep.both).run()
        val sources = transformers.map { flow ⇒
          ...
          Source(Source(fanoutPub).viaMat(flow)(Keep.right).runWith(Sink.publisher(1, 16)))
        }
