...
class FlowTimedSpec extends AkkaSpec with ScriptedTest {
  ...
  "Timed Flow" must {
    ...
    "measure time from start to complete, by wrapping operations" in assertAllStagesStopped {
      ...
      val (flowIn: Subscriber[Int], flowOut: Publisher[String]) = flow.runWith(Source.subscriber[Int], Sink.publisher[String](1, 1))

      val c1 = TestSubscriber.manualProbe[String]()
      val c2 = flowOut.subscribe(c1)

      val p = Source(0 to 100).runWith(Sink.publisher(1, 1))
      p.subscribe(flowIn)
