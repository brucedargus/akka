...
class FlowGroupBySpec extends AkkaSpec {
  ...
  class SubstreamsSupport(groupCount: Int = 2, elementCount: Int = 6) {
    val source = Source(1 to elementCount).runWith(Sink.publisher(1, 1))
    val groupStream = Source(source).groupBy(_ % groupCount).runWith(Sink.publisher(1, 1))
  ...
  "groupBy" must {
    "work in the happy case" in assertAllStagesStopped {
      new SubstreamsSupport(groupCount = 2) {
        val s1 = StreamPuppet(getSubFlow(1).runWith(Sink.publisher(1, 1)))
        ...
        val s2 = StreamPuppet(getSubFlow(0).runWith(Sink.publisher(1, 1)))
    ...
    "accept cancellation of substreams" in assertAllStagesStopped {
      new SubstreamsSupport(groupCount = 2) {
        StreamPuppet(getSubFlow(1).runWith(Sink.publisher(1, 1))).cancel()

        val substream = StreamPuppet(getSubFlow(0).runWith(Sink.publisher(1, 1)))
        substream.request(2)
    ...
    "accept cancellation of master stream when not consumed anything" in assertAllStagesStopped {
      ...
      val publisher = Source(publisherProbeProbe).groupBy(_ % 2).runWith(Sink.publisher(1, 1))
    ...
    "accept cancellation of master stream when substreams are open" in assertAllStagesStopped {
      new SubstreamsSupport(groupCount = 3, elementCount = 13) {
        val substream = StreamPuppet(getSubFlow(1).runWith(Sink.publisher(1, 1)))
    ...
    "work with empty input stream" in assertAllStagesStopped {
      val publisher = Source(List.empty[Int]).groupBy(_ % 2).runWith(Sink.publisher(1, 1))
      val subscriber = TestSubscriber.manualProbe[(Int, Source[Int, _])]()
      publisher.subscribe(subscriber)
    ...
    "abort on onError from upstream when substreams are running" in assertAllStagesStopped {
      ...
      val substreamPuppet = StreamPuppet(substream.runWith(Sink.publisher(1, 1)))
    ...
    "resume stream when groupBy function throws" in {
      ...
      val substreamPuppet1 = StreamPuppet(substream1.runWith(Sink.publisher(1, 1)))
      ...
      val substreamPuppet2 = StreamPuppet(substream2.runWith(Sink.publisher(1, 1)))
