...
class SourceSpec extends AkkaSpec {
  ...
  "Single Source" must {
    "produce element" in {
      val p = Source.single(1).runWith(Sink.publisher(1, 1))
  ...
  "Empty Source" must {
    "complete immediately" in {
      val p = Source.empty.runWith(Sink.publisher(1, 1))
      val c = TestSubscriber.manualProbe[Int]()
      p.subscribe(c)
  ...
  "Failed Source" must {
    "emit error immediately" in {
      ...
      val p = Source.failed(ex).runWith(Sink.publisher(1, 1))
      val c = TestSubscriber.manualProbe[Int]()
      p.subscribe(c)
  ...
  "Maybe Source" must {
    "complete materialized future with None when stream cancels" in {
      ...
      val pubSink = Sink.publisher[Int](1, 1)
