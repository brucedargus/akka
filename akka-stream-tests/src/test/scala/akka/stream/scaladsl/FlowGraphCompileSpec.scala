...
class FlowGraphCompileSpec extends AkkaSpec {
  ...
  val out1 = Sink.publisher[String](1, 1)
  ...
  "A Graph" should {
    ...
    "build wikipedia Topological_sorting" in {
      ...
      RunnableGraph.fromGraph(FlowGraph.create() { implicit b ⇒
        ...
        val out2 = Sink.publisher[String](1, 1)
        val out9 = Sink.publisher[String](1, 1)
        val out10 = Sink.publisher[String](1, 1)
    ...
    "build unzip - zip" in {
      RunnableGraph.fromGraph(FlowGraph.create() { implicit b ⇒
        ...
        val out = Sink.publisher[(Int, String)](1, 1)
        import FlowGraph.Implicits._
    ...
    "distinguish between input and output ports" in {
      intercept[IllegalArgumentException] {
        RunnableGraph.fromGraph(FlowGraph.create() { implicit b ⇒
          val zip = b.add(Zip[Int, String]())
          val unzip = b.add(Unzip[Int, String]())
          val wrongOut = Sink.publisher[(Int, Int)](1, 1)
          val whatever = Sink.publisher[Any](1, 1)
    ...
    "build with implicits and variance" in {
      RunnableGraph.fromGraph(FlowGraph.create() { implicit b ⇒
        ...
        val unzip = b add Unzip[Int, String]()
        val whatever = b add Sink.publisher[Any](1, 1)
        import FlowGraph.Implicits._
        ...
        b.add(Source(apples)) ~> Flow[Apple] ~> b.add(Sink.publisher[Fruit](1, 1))
        appleSource ~> Flow[Apple] ~> merge.in(10)

        Source(List(1 -> "a", 2 -> "b", 3 -> "c")) ~> unzip.in
        unzip.out1 ~> whatever
        unzip.out0 ~> b.add(Sink.publisher[Any](1, 1))
