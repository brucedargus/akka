...
class FlowSplitAfterSpec extends AkkaSpec {
  ...
  class SubstreamsSupport(splitAfter: Int = 3, elementCount: Int = 6) {
    ...
    val groupStream = source.splitAfter(_ == splitAfter).runWith(Sink.publisher(1, 1))
  ...
  "splitAfter" must {
    ...
    "work in the happy case" in assertAllStagesStopped {
      new SubstreamsSupport(3, elementCount = 5) {
        val s1 = StreamPuppet(expectSubFlow().runWith(Sink.publisher(1, 1)))
        ...
        val s2 = StreamPuppet(expectSubFlow().runWith(Sink.publisher(1, 1)))
