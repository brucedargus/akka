...
class GraphBalanceSpec extends AkkaSpec {
  ...
  "A balance" must {
    ...
    "support waiting for demand from all downstream subscriptions" in {
      ...
      val p2 = RunnableGraph.fromGraph(FlowGraph.create(Sink.publisher[Int](1, 1)) { implicit b ⇒
        p2Sink ⇒
    ...
    "support waiting for demand from all non-cancelled downstream subscriptions" in assertAllStagesStopped {
      ...
      val (p2, p3) = RunnableGraph.fromGraph(FlowGraph.create(Sink.publisher[Int](1, 1), Sink.publisher[Int](1, 1))(Keep.both) { implicit b ⇒
        (p2Sink, p3Sink) ⇒
