...
class FlowCompileSpec extends AkkaSpec {
  ...
  "Flow" should {
    ...
    "append Flow" in {
      ...
      val closedSink: Sink[Int, _] = open3.to(Sink.publisher[Int](1, 1))
      "closedSink.run()" shouldNot compile

      closedSource.to(Sink.publisher[Int](1, 1)).run()
      intSeq.to(closedSink).run()
    }
    "append Sink" in {
      val open: Flow[Int, String, _] = Flow[Int].map(_.toString)
      val closedSink: Sink[String, _] = Flow[String].map(_.hashCode).to(Sink.publisher[Int](1, 1))
    ...
    "be appended to Source" in {
      ...
      closedSource2.to(Sink.publisher[String](1, 1)).run
    }
  }

  "Sink" should {
    val openSource: Sink[Int, _] =
      Flow[Int].map(_.toString).to(Sink.publisher[String](1, 1))
    "accept Source" in {
  ...
  "Source" should {
    ...
    "accept Sink" in {
      openSource.to(Sink.publisher[String](1, 1))
    }
  ...
  "RunnableGraph" should {
    ...
    val closed: RunnableGraph[Publisher[String]] =
      Source(Seq(1, 2, 3)).map(_.toString).toMat(Sink.publisher[String](1, 1))(Keep.right)
    "run" in {
      closed.run()
    }
    "not be accepted by Source" in {
