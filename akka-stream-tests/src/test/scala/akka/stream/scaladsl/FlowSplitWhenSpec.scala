...
class FlowSplitWhenSpec extends AkkaSpec {
  ...
  class SubstreamsSupport(splitWhen: Int = 3, elementCount: Int = 6) {
    ...
    val groupStream = source.splitWhen(_ == splitWhen).runWith(Sink.publisher(1, 1))
  ...
  "splitWhen" must {
    ...
    "work in the happy case" in assertAllStagesStopped {
      new SubstreamsSupport(elementCount = 4) {
        val s1 = StreamPuppet(getSubFlow().runWith(Sink.publisher(1, 1)))
        ...
        val s2 = StreamPuppet(getSubFlow().runWith(Sink.publisher(1, 1)))
