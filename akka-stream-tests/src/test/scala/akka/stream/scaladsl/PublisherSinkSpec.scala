...
class PublisherSinkSpec extends AkkaSpec {
  ...
  "A PublisherSink" must {
    ...
    "be unique when created twice" in assertAllStagesStopped {
      ...
      val (pub1, pub2) = RunnableGraph.fromGraph(FlowGraph.create(Sink.publisher[Int](1, 1), Sink.publisher[Int](1, 1))(Keep.both) { implicit b ⇒
        (p1, p2) ⇒
    ...
    "work with SubscriberSource" in {
      val (sub, pub) = Source.subscriber[Int].toMat(Sink.publisher(1, 1))(Keep.both).run()
    ...
    "be able to use Publisher in materialized value transformation" in {
      val f = Source(1 to 3).runWith(
        Sink.publisher[Int](1, 1).mapMaterializedValue(p ⇒ Source(p).runFold(0)(_ + _)))
